	...
	<div class = "pagina-consultas">
			...
			<div class = "contenedor-medio">
				...
				<div class = "formulario">
					<h1 class = "titulo">Alta curso</h1>
					<form  class = "formulario__elementos" action = "" method = "post" id = "formAlta">
						<input class = "formulario__entrada-texto" type="text" name = "curso" id = "curso" placeholder = "Nombre del curso" required />
						<select class = "formulario__selector" name ="aula" id = "aula">
							<option value = ""> Aula </option>
						<?php foreach($this->aulas as $a){ ?>
							<option value = "<?=$a['id_aula'] ?>"> <?= $a['aula'] ?> </option>
						<?php } ?>
						</select>

						<select class = "formulario__selector" name ="profesor" id = "profesor">
							<option value = ""> Profesor </option>
						<?php foreach($this->profesores as $a){ ?>
							<option value = "<?=$a['id_profesor'] ?>"> <?= $a['prof_apellido'] . ", " . $a['prof_nombre'] ?> </option>
						<?php } ?>
						</select>
